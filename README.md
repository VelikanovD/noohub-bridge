...
локальный HTTP API
        |
noohub-bridge на Wiren Board
        |
виртуальные устройства Wiren Board
        |
отдельный MQTT-брокер Wiren Board :45883
        |
      SprutHub
```
...
- Для устройств без обратной связи (`retrievable` не `true`) при старте отправляет `off`, выставляет состояние `off` в карточке Wiren Board, а дальше Wiren Board хранит состояние по успешно отправленным командам.
- Обычные устройства без обратной связи отображаются переключателем `On`, потому что состояние хранится в Wiren Board; явные NooLite Impulse остаются кнопкой.
